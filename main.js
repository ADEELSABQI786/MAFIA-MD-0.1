...
async function start() {
  try {
    ...
    console.log("MAFIA-MD is running on v" + _0x2f6d2f.join('.') + ", isLatest: " + _0x23f0a4);
    const _0x76bf4 = makeWASocket({
      ...
      'getMessage': async _0x53ca5a => {
        ...
        return {
          'conversation': "BEST WHATSAPP BOT MADE BY MAFIA ADEEL"
        };
      }
    });
    _0x76bf4.ev.on("connection.update", _0x4eb449 => {
      ...
      if (_0x237ed1 === "close") {
        ...
        if (initialConnection) {
          console.log(_0x4b0fc4.green("MAFIA-MD MD CONNECTED SUCCESSFULLY ✅"));
          _0x76bf4.sendMessage(_0x76bf4.user.id, {
            'text': "╭─────────────━┈⊷\n│ *MAFIA ɪs ᴄᴏɴɴᴇᴄᴛᴇᴅ*\n╰─────────────━┈⊷\n\n╭─────────────━┈⊷\n│🤖 ʙᴏᴛ ɴᴀᴍᴇ: *MAFIA-ᴍᴅ*\n│👨‍💻 ᴏᴡɴᴇʀ : *MAFIA ADEEL*\n╰─────────────━┈⊷\n\n*Message Me on whatsapp 😈*\n_https://wa.me/923131613251_"
          });
...
async function init() {
  if (_0x5687e2.existsSync(credsPath)) {
    ...
    if (_0x17d9d4) {
      console.log("MAFIA-MD IS RUNNING...⏳");
      await start();
    } else {
...
app.get('/', (_0x1ecf21, _0x282bcc) => {
  _0x282bcc.send("MAFIA-MD IS CONNECTED SUCCESSFULLY ✅");
});
app.listen(PORT, () => {
  console.log("MAFIA-MD daily users " + PORT);
});
